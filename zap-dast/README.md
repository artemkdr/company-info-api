...
│   ├── zap-af-enriched.yaml      # Pass 2: OpenAPI-guided, alertFilters, authenticated
│   ├── openapi-params.json       # Test data for parameter enrichment (reserved)
│   └── openapi-enriched.json     # Generated by generate-openapi.sh
├── scripts/
│   ├── generate-openapi.sh       # Generate OpenAPI 3.0 spec
...
2. **Docker Compose** — boot target, mock-upstreams, zap-auth-proxy
3. **Generate OpenAPI** — create enriched spec with example values
4. **Preflight** — validate containers, network isolation, AF YAML syntax
5. **Baseline Scan** — unauthenticated spider + activeScan, generates HTML + JSON reports
6. **Enriched Scan** — import OpenAPI spec, apply alertFilters, authenticated scan via proxy
7. **Triage** — apply threat-model rules to enriched.json, determine pass/fail (fails on High/Medium, not Low/Info)
8. **Upload Artifacts** — save HTML + JSON reports + triaged report as build artifact
9. **Cleanup** — stop and remove containers

Exit code from triage determines CI pass/fail.
